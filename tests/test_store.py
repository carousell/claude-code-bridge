...
from claude_code_bridge.store import TaskRecord
...
def test_live_session_ids_spans_processes(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr(store, "process_alive", lambda pid, session: pid == 1)
...
def test_a_terminal_record_with_no_exit_code_is_rechecked_against_the_process(
    ...
    Observed in the wild: a client restarted the bridge, both in-flight tasks were recorded
    `failed` with no exit code, and the `claude` processes carried on working for another ten
    minutes while every tool reported them dead.
    """
...
def test_live_session_ids_does_not_probe_settled_tasks(
    ...
    def counting_alive(pid, session):
        probed.append(pid)
        return False
