#!/usr/bin/env bash
# Install claude-code-bridge and register it with the Claude desktop app.
#
...
# `claude-code-bridge-setup`, which is Python and covered by tests.
#
...
[ "$#" -eq 0 ] || die "install.sh takes no arguments (got: $*). For options, run claude-code-bridge-setup directly."
[ -f "$repo_root/pyproject.toml" ] || die "run this from a claude-code-bridge checkout"
...
command -v claude-code-bridge-setup >/dev/null 2>&1 \
  || die "installed, but claude-code-bridge-setup is not on PATH. Add $(uv tool dir --bin 2>/dev/null || echo "$HOME/.local/bin") to your PATH."
...
Claude Code itself is not installed, and the bridge dispatches to it.
