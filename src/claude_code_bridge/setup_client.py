"""Register the installed bridge server with the Claude desktop app.

The reason this exists rather than a README snippet: a GUI-launched process does not inherit the
shell `PATH`, so a config that just says `"command": "claude-code-bridge-server"` fails twice over
— the executable is not found, and even when it is, the server cannot find the `claude` binary it
dispatches to. Both need absolute, resolved paths, and getting that wrong produces an error nobody
...
from .cli import CLAUDE_BINARY

SERVER_KEY = "claude-code-bridge"
SERVER_COMMAND = "claude-code-bridge-server"
...
def setup(config_path: Path, *, dry_run: bool = False) -> dict[str, Any]:
    ...
    entry = server_entry(server_command, shutil.which(CLAUDE_BINARY), shutil.which("git"))
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        prog="claude-code-bridge-setup",
        description="Register claude-code-bridge with the Claude desktop app.",
    )
    parser.add_argument(
        "--config",
        type=Path,
    ...
    if shutil.which(CLAUDE_BINARY) is None:
        print(
            f"\nwarning: `{CLAUDE_BINARY}` is not on PATH, so dispatching will fail until Claude "
            "Code is installed. Install it, then re-run this command to record its location.",
            file=sys.stderr,
        )
