...
def process_alive(pid: int | None, session_id: str) -> bool:
    """Whether the recorded process is still running *and* is still the task we think it is.

    The pid alone is not enough: it may have been reused since. Every dispatch carries its session
    id on the command line (`--session-id` or `--resume`), which makes for a cheap identity check.
    """
    if pid is None:
        return False
    try:
...
def replay_log(log_dir: Path, task_id: str) -> tuple[StreamState, list[str]]:
...
def outcome_unobserved(record: TaskRecord) -> bool:
    ...
    its monitor crashes. The `claude` process survives both, so these records have to be rechecked
    against the process rather than believed.
    """
    return record.status == "running" or record.exit_code is None


def resolve_status(log_dir: Path, record: TaskRecord) -> tuple[str, str, StreamState, list[str]]:
    ...
    reported as another.
    """
    state, tail = replay_log(log_dir, record.task_id)
    unobserved = outcome_unobserved(record)
    alive = unobserved and process_alive(record.pid, record.session_id)
    ...
    if state.result_event is not None:
        status = (
            "timed_out"
            if state.subtype == "error_max_turns"
            else ("failed" if state.is_error else "completed")
        )
        return (
            status,
...
def live_session_ids(log_dir: Path) -> set[str]:
    ...
    return {
        ...
        if outcome_unobserved(record) and process_alive(record.pid, record.session_id)
    }
